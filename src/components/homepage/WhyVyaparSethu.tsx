  ...
  return (
    <section className="py-14 px-4 bg-[#0a1128] border-t border-white/5">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Comparison table */}
        <p className="text-xs text-slate-500 italic mb-1 sm:hidden text-center">← Scroll to compare →</p>
          ...
          <table className="w-full min-w-[540px] text-sm">
            <thead>
              <tr className="bg-[#001f3f] border-b border-slate-700/60">
                ...
                <th className="text-left text-slate-500 font-medium px-4 py-3 text-xs uppercase tracking-wider">IndiaMART</th>
                <th className="text-left text-slate-500 font-medium px-4 py-3 text-xs uppercase tracking-wider">TradeIndia</th>
              </tr>
            </thead>
            <tbody>
              {comparisonRows.map((row, i) => (
                <tr key={row.feature} className={i % 2 === 0 ? 'bg-slate-800/25' : 'bg-slate-800/10'}>
                  <td className="px-4 py-2.5 text-slate-400 text-xs font-medium">{row.feature}</td>
                  <td className="px-4 py-2.5 text-white text-xs font-semibold">{row.vs}</td>
                  <td className="px-4 py-2.5 text-slate-500 text-xs">{row.indiamart}</td>
                  <td className="px-4 py-2.5 text-slate-500 text-xs">{row.tradeindia}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="text-slate-600 text-xs italic mb-10">
