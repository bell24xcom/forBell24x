...
interface Props {
  /**
   * Public, non-personal fields fetched server-side (see page.tsx) — seeds
   * the very first render so the crawlable HTML already has real content
   * instead of the loading spinner. The post-mount fetch below replaces
   * this with the full record (including buyer info) for the interactive UI.
   */
  initialRFQ: Record<string, unknown>;
}

export default function RFQDetailClient({ initialRFQ }: Props) {
