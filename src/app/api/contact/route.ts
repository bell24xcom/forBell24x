...
export async function POST(request: NextRequest) {
  try {
    ...
    const subjectLine = subject
      ? `[VyaparSethu Contact] ${subject}`
      : `[VyaparSethu Contact] ${inquiryType || 'General'} inquiry from ${name}`;

    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: #1E40AF; color: white; padding: 20px 24px;">
          <h2 style="margin: 0;">New Contact Form Submission</h2>
          <p style="margin: 4px 0 0; opacity: 0.8; font-size: 14px;">VyaparSethu Contact Form</p>
        </div>
        ...
        <div style="padding: 12px 24px; background: #f1f5f9; font-size: 12px; color: #64748b;">
          VyaparSethu · Digitex Studio · GSTIN: 27AAAPP9753F2ZF
        </div>
      </div>
    `;
