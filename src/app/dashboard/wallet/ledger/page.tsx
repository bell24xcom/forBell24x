...
export default function DashboardWalletLedgerPage() {
  ...
  const exportCSV = () => {
    const headers = 'Date,Type,Description,Amount (INR),Balance After (INR)\n';
  ...
  return (
    <div>
      <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Current balance: <span className="text-white font-semibold">{fmt(balance)}</span>
          </p>
        </div>
      ...
      {error && (
        <div className="bg-red-900/30 border border-red-700/50 text-red-300 rounded-lg px-4 py-3 mb-6 text-sm">
          {error}
        </div>
      )}
                    ...
                    <tr key={tx.id} className="hover:bg-slate-700/20 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-semibold whitespace-nowrap">
                        <span className="flex items-center justify-end gap-1">
                          {credit ? <ArrowDownLeft className="w-3 h-3 text-green-400" /> : <ArrowUpRight className="w-3 h-3 text-red-400" />}
