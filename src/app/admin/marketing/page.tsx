...
import { 
  RefreshCw, IndianRupee, Tag, Package, Sparkles, 
  MessageSquare, User, CheckCircle2, ChevronDown, 
  ChevronUp, Send, Eye, ShieldCheck, Activity
} from 'lucide-react';
...
export default function RevenueControlPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch quotes for all RFQs (In production, fetch lazily on expand)
      const quotesRes = await fetch('/api/admin/marketing/quotes');
      const quotesData = await quotesRes.json();
      
      // Group quotes by RFQ ID
      const grouped = quotesData.reduce((acc: any, q: Quote) => {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500">Initializing Revenue Control...</div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-indigo-600/10 border border-indigo-500/30 p-6 rounded-3xl">
          ...
          <h2 className="text-3xl font-bold text-white">
            {rfqs.length > 0 ? ((rfqs.filter(r => r.status === 'closed').length / rfqs.length) * 100).toFixed(1) : 0}%
          </h2>
        </div>
        <div className="bg-emerald-600/10 border border-emerald-500/30 p-6 rounded-3xl">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-emerald-500 text-sm font-bold uppercase">LIVE</span>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-800/50">
              {rfqs.map(rfq => (
                <React.Fragment key={rfq.id}>
                  <tr className={`hover:bg-slate-800/30 transition-colors cursor-pointer ${expandedRfq === rfq.id ? 'bg-indigo-500/5' : ''}`} onClick={() => setExpandedRfq(expandedRfq === rfq.id ? null : rfq.id)}>
                    ...
                    <td className="px-6 py-4 text-right">
                      {expandedRfq === rfq.id ? <ChevronUp className="w-4 h-4 ml-auto" /> : <ChevronDown className="w-4 h-4 ml-auto" />}
                    </td>
                  </tr>
                  {expandedRfq === rfq.id && (
                    <tr>
