...
export async function storeRFQ(meta: RFQMeta): Promise<void> {
  ...
  // BOM activation: record a BusinessLifeEvent for this RFQ so the Business
  // Operating Memory (Morning Brief, Company DNA, genome score) is fed real
  // signals. companyId is passed by callers via meta.metadata.companyId or
  // a top-level companyId field. Non-blocking — never fails the RFQ write.
  const companyId =
    (meta as { companyId?: string }).companyId ??
    (meta.metadata?.companyId as string | undefined);
  if (companyId) {
    ...
    try {
      ...
      console.error('[Memory] storeRFQ life-event failed:', err instanceof Error ? err.message : err);
    }
  }
}
...
export async function storeQuote(meta: QuoteMeta): Promise<void> {
  ...
  // BOM activation: record a quote_received life event for both the supplier
  // (their quote) and the buyer (a quote landed on their RFQ). Non-blocking.
  try {
    ...
    console.error('[Memory] storeQuote life-event failed:', err instanceof Error ? err.message : err);
  }
}
