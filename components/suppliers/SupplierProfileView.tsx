...
export default function SupplierProfileView({ 
  ...
  return (
    <div className="min-h-screen bg-[#0F172A]">
      {/* Hero Section */}
      <div className="relative bg-slate-900 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col lg:flex-row gap-8">
            {/* Company Logo */}
            <div className="relative">
              <div className="w-32 h-32 lg:w-40 lg:h-40 relative rounded-lg overflow-hidden bg-slate-800">
                {supplier.logo_url ? (
                  <Image
                ...
                {isOwner && (
                  <button className="absolute bottom-2 right-2 bg-slate-700 rounded-full p-2 shadow-md hover:shadow-lg transition-shadow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {supplier.employee_count && (
              <div>
                ...
                <p className="text-lg font-medium text-white">{supplier.employee_count} employees</p>
              </div>
            )}
            {supplier.annual_turnover && (
              <div>
                <p className="text-sm text-slate-400">Annual Turnover</p>
                <p className="text-lg font-medium text-white">{supplier.annual_turnover}</p>
              </div>
            )}
            {supplier.gst_number && (
              <div>
                <p className="text-sm text-slate-400">GST Number</p>
                <p className="text-lg font-medium text-white">{supplier.gst_number}</p>
              </div>
            )}
          </div>
        </Card>
